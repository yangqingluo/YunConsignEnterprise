...
@interface WaybillCountDetailVC ()

@property (strong, nonatomic) PublicMutableLabelView *footerView;

@end

@implementation WaybillCountDetailVC
...
- (void)setupNav {
    [self createNavWithTitle:@"运单统计" createMenuItem:^UIView *(int nIndex){
        if (nIndex == 0){
...
- (void)pullBaseListData:(BOOL)isReset {
    ...
    if (self.condition) {
        ...
        if (self.condition.start_service) {
            [m_dic setObject:self.condition.start_service.service_id forKey:@"start_station_service_id"];
        }
        if (self.condition.end_service) {
            [m_dic setObject:self.condition.end_service.service_id forKey:@"end_station_service_id"];
        }
        if (self.condition.real_station_city_name) {
            [m_dic setObject:self.condition.real_station_city_name forKey:@"real_station_city_name"];
        }
    ...
    QKWEAKSELF;
    [[QKNetworkSingleton sharedManager] commonSoapPost:@"hex_finance_queryWaybillListByConditionFunction" Parm:m_dic completion:^(id responseBody, NSError *error){
...
- (void)pullBaseTotalData:(BOOL)isReset parm:(NSDictionary *)parm {
    ...
    [self doShowHudFunction];
    QKWEAKSELF;
    [[QKNetworkSingleton sharedManager] commonSoapPost:@"hex_finance_queryWaybillTotalByConditionFunction" Parm:parm completion:^(id responseBody, NSError *error){
        [weakself endRefreshing];
        if (!error) {
            ResponseItem *item = responseBody;
            if (item.flag == 1) {
