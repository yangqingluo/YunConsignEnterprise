...
@interface ServiceNoteVC ()

@end

@implementation ServiceNoteVC
...
- (void)addBtnAction {
    SaveServiceNoteVC *vc = [SaveServiceNoteVC new];
    [self goToSaveVC:vc];
}
...
- (void)pullBaseListData:(BOOL)isReset {
    ...
    [[QKNetworkSingleton sharedManager] commonSoapPost:@"hex_base_queryServiceNoteList" Parm:m_dic completion:^(id responseBody, NSError *error){
        ...
        if (!error) {
            ...
            [weakself.dataSource addObjectsFromArray:[AppNoteInfo mj_objectArrayWithKeyValuesArray:item.items]];
...
- (void)doRemovingDataAtIndexPath:(NSIndexPath *)indexPath {
    ...
    AppNoteInfo *item = self.dataSource[indexPath.row];
    NSMutableDictionary *m_dic = [NSMutableDictionary dictionaryWithDictionary:@{@"note_id" : item.note_id}];
    [self doShowHudFunction];
    QKWEAKSELF;
    [[QKNetworkSingleton sharedManager] commonSoapPost:@"hex_base_deleteServiceNoteById" Parm:m_dic completion:^(id responseBody, NSError *error){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    AppNoteInfo *item = self.dataSource[indexPath.row];
    cell.baseView.textLabel.text = item.note_info;
